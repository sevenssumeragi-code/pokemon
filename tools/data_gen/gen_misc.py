...
# ---- RPG items (medicine / balls / evolution / key) ----
item("potion","キズぐすり",pocket="medicine",price=200,effect={"heal_hp":20})
item("super_potion","いいキズぐすり",pocket="medicine",price=700,effect={"heal_hp":60})
item("hyper_potion","すごいキズぐすり",pocket="medicine",price=1500,effect={"heal_hp":120})
item("max_potion","まんたんのくすり",pocket="medicine",price=2500,effect={"heal_hp":9999})
item("full_heal","なんでもなおし",pocket="medicine",price=400,effect={"cure_status":"all"})
item("antidote","どくけし",pocket="medicine",price=200,effect={"cure_status":"psn"})
item("awakening","ねむけざまし",pocket="medicine",price=200,effect={"cure_status":"slp"})
item("revive","げんきのかけら",pocket="medicine",price=2000,effect={"revive":0.5})
item("ether","ピーピーエイド",pocket="medicine",price=1200,effect={"restore_pp":10})
item("monster_ball","モンスターボール",pocket="ball",price=200,effect={"ball_rate":1.0})
item("great_ball","スーパーボール",pocket="ball",price=600,effect={"ball_rate":1.5})
item("ultra_ball","ハイパーボール",pocket="ball",price=1200,effect={"ball_rate":2.0})
item("mind_stone","しんぴのいし",pocket="evolution",price=3000,effect={"evolve":True})
item("region_charm","リージョンのおまもり",pocket="key",price=0,effect={"key":True})
item("town_map","タウンマップ",pocket="key",price=0,effect={"key":True})
for k in items:
    items[k].setdefault("flags", {})
    items[k].setdefault("is_berry", False)
    items[k].setdefault("params", {})
    items[k].setdefault("pocket", "held")
    items[k].setdefault("price", 1000 if items[k]["pocket"] == "held" else 0)
    if items[k]["pocket"] != "held": items[k].setdefault("handler", "passive")
