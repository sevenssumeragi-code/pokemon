...
## 現在フェーズ
Phase 4 完了（通しテスト合格）→ Phase 5（ダブルバトル→マップ・ストーリー追加）着手。Phase 3/4 の人間プレイ確認はユーザー環境で。
...
- [x] マップデータ形式（data/maps/*.json + tileset.json）・MapData・EventRunner（data/events/*.json）・トレーナー（data/trainers）
- [x] 探索シーン RPGController（グリッド移動・ワープ・NPC対話・看板・草むらエンカウント・自動イベント）、MapRenderer（矩形描画・カメラ追従）
- [x] RPG UI: DialogBox（メッセージ/選択肢）、RPGMenu（パーティ/バッグ/ずかん/セーブ/設定/ショップ）、戦闘画面のアイテム・にげる・捕獲
- [x] BattleFlow: 経験値（Gen5式・参加者分配）、レベルアップ技習得（忘れる技の選択）、進化（Lv／しんぴのいし）、捕獲→手持ち/ボックス、全滅→町へ
- [x] コンテンツ: はじまりの町（家・研究所・ショップ・回復NPC）・みどりの草原（トレーナー2）・リージョンのほら穴（リージョンフォーム野生・ボス）・エンディング
- [x] 自動通しテスト（tests/rpg_playthrough.gd / run_playthrough.sh）: ニューゲーム→御三家→草原トレーナー2人→ほら穴→ボス→エンディングまで PLAYTHROUGH_OK（レニィLv23で到達）

## 操作
- 探索: 矢印/WASD 移動、Z/Enter/Space 決定・話す、X/Esc メニュー。戦闘: マウス（ボタン）。
- 通しテスト: `./run_playthrough.sh`（数分。PLAYTHROUGH_OK で成功）

## Phase 5 進捗
- [x] ダブルバトル: エンジンテスト8件（全体技0.75倍、味方への炎の領域/やみのりょういき、いかく2体、対象再選択、片側ひんし交代、てだすけ、AI完走）
- [x] 戦闘UI: 2枠ずつの表示、行動するモンスターごとのコマンド、対象選択パネル、チームビルダーの「ダブルバトル」チェック
- [x] シミュレーター mode=doubles（4体チーム）— (c) 検証 8000戦完了（reports/balance_r10_doubles.md, reports/phase5_doubles_summary.md）: 領域特性でヒュウ61.5/ジンパチ60.1、調整はユーザー判断待ち
- [x] マップ追加: うみべのみち / みなとまち / バトルホール（ボス撃破後に東門が開く）、トレーナー4人（うち2人はダブル形式）、ライバル戦
- [x] ポストゲーム通しテスト（`POSTGAME=1 ./run_playthrough.sh`）: エンディングのセーブから続き、ライバル撃破→バトルホール（ダブル）撃破まで PLAYTHROUGH_OK（ボットはレニィ単騎のためLv43まで育成して突破。人間は複数体で挑む想定）

## 次の具体的タスク3つ
1. ユーザー判断: ダブルでの領域特性（味方への倍率）を下げるか（BALANCE_LOG 末尾の案）
2. （完了）ポストゲーム通しテスト → 次: ボットに捕獲個体の育成を教えて難易度感を再確認
3. 人間プレイ確認のフィードバック待ち（Phase 3/4/5）。残: PP持ち越し、BGM/SE、画像差し替え（assets/monsters）
